...
// Constructor
CangrejoPinzaBrazo::CangrejoPinzaBrazo()
{
...
// Crea un objeto
void CangrejoPinzaBrazo::create()
{
	...
	float ancho_pc0y = 0.3;

	float ancho_pc1x = 8.0;
	float ancho_pc1y = 0.1;

	float ancho_pc2x = 9.0;
	float ancho_pc2y = 0.0;
	...
	for(int q = 0; q < this->ESTIRAMIENTO; q++)
	{
		...
		float pc0y = -1.0 * forma + ancho;
		float pc0z = 0.0;

		float pc1x = -1.0 * forma;
		float pc1y = -1.0 * forma + ancho;
		float pc1z = 0.0;

		float pc2x = -1.0 * forma;
		float pc2y = 1.0 * forma + ancho;
		float pc2z = 0.0;

		float pc3x = 1.0 * forma;
		float pc3y = 1.0 * forma + ancho;
		float pc3z = 0.0;
...
void CangrejoPinzaBrazo::render(glm::mat4 model_matrix, glm::mat4 &view_matrix, 
